...
3. 单调段内仅比较相邻点 单调段内仅比较相邻点
4. 极点附近做局部跨段比较 极点附近做局部跨段比较
5. 滑动窗口保证全局正确性 滑动窗口保证全局正确性
6. 全程无递归，速度远超传统分治法 全程无递归，速度远超传统分治法

## 算法原理与正确性
1. 点按 x 坐标升序排列。 点按 x 坐标升序排列。
2. 根据 y 的上升/下降趋势，将序列划分为 根据 y 的上升/下降趋势，将序列划分为**单调段**，分段点为，分段点为**极点（峰值/谷值）**。。
3. **单调段内**：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。：最小距离一定出现在相邻点之间，只需线性遍历。
4. **极点附近**：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。：仅需局部检查，即可覆盖跨段最近点对。
5. **滑动窗口**：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。：对远距离分段进行剪枝检查，确保 100% 正确且无冗余计算。
6. 所有可能的最近点对均被覆盖，结果严格正确。 所有可能的最近点对均被覆盖，结果严格正确。

![算法分段示意图](image/单调分段证明.png)
![极点跨段比较](image/山峰山谷最小值证明.png)

## 时间复杂度
- 排序：O(n log n) 排序：O(n log n)
- 分段 + 段内遍历：O(n) 分段 + 段内遍历：O(n)
- 极点检查 + 滑动窗口：O(n) 极点检查 + 滑动窗口：O(n)
- **整体复杂度：O(n log n)**
- **实际运行效率：远优于标准分治法**

## 核心函数说明
- `extremum()`：极点（峰值/谷值）附近跨段点对检查：极点（峰值/谷值）附近跨段点对检查
- `checkSegments()`：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性：滑动窗口全局剪枝，保证正确性
- `findClosestPair()`：算法主入口：算法主入口：算法主入口：算法主入口：算法主入口：算法主入口：算法主入口：算法主入口：算法主入口：算法主入口：算法主入口：算法主入口：算法主入口：算法主入口：算法主入口：算法主入口
...
## 使用方法
输入点的数量 n，程序自动生成随机点并计算最小点对距离。输入点的数量 n，程序自动生成随机点并计算最小点对距离。
